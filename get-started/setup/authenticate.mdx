---
title: "2. Authenticate and Initialize"
description: "Autheticate your logged in users with the SDK. Then initialize a collaboritive space within your app to use Velt"
---
...
<Tabs>
 ...
 ## 1) Create a component that will represent your multi-collaboritive space.

Create a component that will represent your multi-collaboritive space.
...
Users logged into the same document ID can see each other's presence, cursors, comments etc. You can create multiple documents within your application to create separate collaboritive spaces.
...
export default function YourComponent() {
  ...
  useEffect(() => {
    if (client && yourAuthenticatedUser) {
      ...
      //6) Initialize your document with a unique ID
      client.setDocumentId('unique-document-id');
    }
  }, [client, yourAuthenticatedUser]);
