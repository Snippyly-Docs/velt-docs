...
To add multiple locations that are meant to be active at the same time, use the `client.setLocation()` method and pass in the location object you want to add as the first parameter and `true` as the second parameter. 
...
<Warning>
Be sure to a unique id attribute on each of the containers.
</Warning>
...
<div class="container">
  ...
  <div id="container-id" class="sub-container" data-velt-location={JSON.stringify({key1: "value1"})}>
    <div class="card">
      ... any content ...
    </div>
  </div>
