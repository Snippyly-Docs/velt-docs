...
In **YourDocument.js**, add the `VeltCommentsTool`to test out the `Comments` functionality.
...
### Comments 
- Click the `VeltCommentsTool` button, then hover over any element on the page to leave a comment.
- Click the `VeltCommentsTool` button, then try to draw a box on the page to leave a comment.
