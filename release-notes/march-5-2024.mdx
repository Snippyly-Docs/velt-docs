...
## Method to return XPath of DOM element where a Comment was added.

Introducing the [getElementRefByAnnotationId() method](/async-collaboration/comments/customize-behavior/action-methods) that returns the XPath of the DOM element where the specified comment was added.
...
Added the [scrollToCommentByAnnotationId() method](/async-collaboration/comments/customize-behavior/action-methods) that scrolls the page to the specified element where the comment was added. This functionality is contingent upon the presence of the element in the DOM.
...
Implemented the [onPresenceUserClick event handler](/realtime-collaboration/presence/customize-behavior) for click events on Presence avatar circles.
