...
In your server endpoint, call our `https://generateveltauthtoken.api.velt.dev` endpoint to generate a `JWT Token`.
...
### Request Body for `https://generateveltauthtoken.api.velt.dev`:
...
<Info>
To learn more about generating an Auth Token from the Velt Console, [read here](/security/auth-tokens).
</Info>


### Success Response for `https://generateveltauthtoken.api.velt.dev`:
...
### Failure Response for `https://generateveltauthtoken.api.velt.dev`:
...
export default function YourAuthComponent() {
  ...
  async funtion callToYourServerToGetJWTToken(userId){
