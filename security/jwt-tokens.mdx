...
The `generateVeltAuthToken()` method shown above does a fetch call to `https://generateveltauthtoken.api.velt.dev` with the following body:

- `userId` - unique user id of the user you are generating a JWT Token for
- `apiKey` - Velt API KEy
- `authToken` - Auth Token from the Velt Console

The following respone object will be returned from the fetch call:
