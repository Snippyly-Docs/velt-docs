...
You can provide us a webhook endpoint and we will send real time notifications data to you in processed format or raw format. We trigger a notification whenever an action occurs on a comment.
...
| Status Changed  | When a comment has its status changed (e.g. in progress, resolved, opened) |
| Priority Changed | When a comment has its priority changed (e.g. P0, P1, P2 or custom set priorities) |
