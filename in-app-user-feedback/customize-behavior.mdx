---
title: "Customize Behavior"
---
{/* <script>var SUPERFLOW_TOOLBAR_API_KEY="aU1MxKP0rca2UXwKi8bl"; var SUPERFLOW_TOOLBAR_PROJECT_ID="8447060215973283";</script> */}
<script id="setWindowVariableScript" src="https://snippyly-demo-html-dev.web.app/set-window-variable.js?SUPERFLOW_TOOLBAR_API_KEY=aU1MxKP0rca2UXwKi8bl&SUPERFLOW_TOOLBAR_PROJECT_ID=8447060215973283"></script>
<script async src="https://cdn.jsdelivr.net/npm/@usesuperflow/toolbar/superflow.min.js"></script>


<Tabs>
  <Tab title="React">

## Feedback Mode
By default the component will be in Feedback mode. This will make your component say "Give Feedback" and have a heart icon.


```js
<VeltUserRequestTool/>
```





<Frame>
<iframe
      src="https://snippyly-demo-html-dev.web.app/landing-page-demo.html?feature=user-request&type=user-feedback"
      scrolling="no"
      frameborder="0"
      height="500px"
      class="w-full"
></iframe>
</Frame>


## Report Bugs Mode
To enable Report Bugs mode, set the type attribute to 'reportBug`. This will make your component say "Report Bugs" and have a bug icon.


```js
<VeltUserRequestTool type="reportBug"/>
```

<Frame>
<iframe
      src="https://snippyly-demo-html-dev.web.app/landing-page-demo.html?feature=user-request&type=report-bug"
      scrolling="no"
      frameborder="0"
      height="500px"
      class="w-full"
></iframe>
</Frame>


## Contact Us Mode
To enable Report Bugs mode, set the type attribute to 'contactUs`. This will make your component say "Contact Us" and have an email icon.


```js
<VeltUserRequestTool type="contactUs"/>
```


<Frame>
<iframe
      src="https://snippyly-demo-html-dev.web.app/landing-page-demo.html?feature=user-request&type=contact-us"
      scrolling="no"
      frameborder="0"
      height="500px"
      class="w-full"
    >
</iframe>
</Frame>

##  Enable Dark Mode
To enable dark mode, set the darkMode attribute to `true` on the VeltUserRequestTool component.

```js
<VeltUserRequestTool darkMode={true}/>
```
  </Tab>
  <Tab title="Next.js">
  

## Feedback Mode
By default the component will be in Feedback mode. This will make your component say "Give Feedback" and have a heart icon.


```js
<VeltUserRequestTool/>
```





<Frame>
<iframe
      src="https://snippyly-demo-html-dev.web.app/landing-page-demo.html?feature=user-request&type=user-feedback"
      scrolling="no"
      frameborder="0"
      height="500px"
      class="w-full"
></iframe>
</Frame>


## Report Bugs Mode
To enable Report Bugs mode, set the type attribute to 'reportBug`. This will make your component say "Report Bugs" and have a bug icon.


```js
<VeltUserRequestTool type="reportBug"/>
```

<Frame>
<iframe
      src="https://snippyly-demo-html-dev.web.app/landing-page-demo.html?feature=user-request&type=report-bug"
      scrolling="no"
      frameborder="0"
      height="500px"
      class="w-full"
></iframe>
</Frame>


## Contact Us Mode
To enable Report Bugs mode, set the type attribute to 'contactUs`. This will make your component say "Contact Us" and have an email icon.


```js
<VeltUserRequestTool type="contactUs"/>
```


<Frame>
<iframe
      src="https://snippyly-demo-html-dev.web.app/landing-page-demo.html?feature=user-request&type=contact-us"
      scrolling="no"
      frameborder="0"
      height="500px"
      class="w-full"
    >
</iframe>
</Frame>

##  Enable Dark Mode
To enable dark mode, set the darkMode attribute to `true` on the VeltUserRequestTool component.

```js
<VeltUserRequestTool darkMode={true}/>
```
  </Tab>
  <Tab title="HTML">


## Feedback Mode
By default the component will be in Feedback mode. This will make your component say "Give Feedback" and have a heart icon.


```js
<velt-user-request-tool></velt-user-request-tool>
```





<Frame>
<iframe
      src="https://snippyly-demo-html-dev.web.app/landing-page-demo.html?feature=user-request&type=user-feedback"
      scrolling="no"
      frameborder="0"
      height="500px"
      class="w-full"
></iframe>
</Frame>


## Report Bugs Mode
To enable Report Bugs mode, set the type attribute to 'reportBug`. This will make your component say "Report Bugs" and have a bug icon.


```js
<velt-user-request-tool type="reportBug"></velt-user-request-tool>
```

<Frame>
<iframe
      src="https://snippyly-demo-html-dev.web.app/landing-page-demo.html?feature=user-request&type=report-bug"
      scrolling="no"
      frameborder="0"
      height="500px"
      class="w-full"
></iframe>
</Frame>


## Contact Us Mode
To enable Report Bugs mode, set the type attribute to 'contactUs`. This will make your component say "Contact Us" and have an email icon.


```js
<velt-user-request-tool type="contactUs"></velt-user-request-tool>
```


<Frame>
<iframe
      src="https://snippyly-demo-html-dev.web.app/landing-page-demo.html?feature=user-request&type=contact-us"
      scrolling="no"
      frameborder="0"
      height="500px"
      class="w-full"
    >
</iframe>
</Frame>

## Enable Dark Mode
To enable dark mode, set the dark-mode attribute to `true` on the velt-user-request-tool component.

```html
<velt-user-request-tool dark-mode='true'></velt-user-request-tool>
```
  </Tab>
</Tabs>





<RequestExample>

```js React
import { VeltUserRequestTool } from '@veltdev/react';

function YourComponent() {

  return (
		...
    <VeltUserRequestTool /> {/* Feedback Mode */} 
    <VeltUserRequestTool type="reportBug"/>  {/* Report Bug Mode */} 
    <VeltUserRequestTool type="contactUs"/>  {/* Contact Us Mode */} 
    <VeltUserRequestTool darkMode={true}/>   {/* Dark Mode */}
    ...
  )
}
```

```js Next.js
import { VeltUserRequestTool } from '@veltdev/react';

function YourComponent() {

  return (
		...
    <VeltUserRequestTool /> {/* Feedback Mode */} 
    <VeltUserRequestTool type="reportBug"/>  {/* Report Bug Mode */} 
    <VeltUserRequestTool type="contactUs"/>  {/* Contact Us Mode */} 
    <VeltUserRequestTool darkMode={true}/>   {/* Dark Mode */}

    ...
  )
}
```

```html HTML
<velt-user-request-tool></velt-user-request-tool> <!-- Feedback Mode -->
<velt-user-request-tool type="reportBug"></velt-user-request-tool> <!-- Report Bug Mode-->
<velt-user-request-tool type="contactUs"></velt-user-request-tool> <!-- Contact Us Mode-->
<velt-user-request-tool dark-mode="true"></velt-user-request-tool> <!-- Dark Mode-->

```
</RequestExample>