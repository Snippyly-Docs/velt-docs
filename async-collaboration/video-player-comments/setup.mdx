...
## 7. Subscribe to when the Comment Tool is activated by the User. 
...
## 9. When the player is played, remove Velt `Location` to remove comments from the media.
...
## 10. Set your player's state when the user clicks on the comment.
