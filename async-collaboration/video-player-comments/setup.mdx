...
Enable TODO features on your Comments.

```jsx
<VeltComments priority={true} autoCategorize={true} commentIndex={true} onCommentAdd={(event) => onCommentAdd(event)} />
```
...
Enable embedMode and handle click events on the comments in the sidebar.
...
You can subscribe to changes in the comment mode using the `onCommentModeChange().subscribe()` method.

You can use this to pause your video player whenever a new comment is made.

```jsx
commentElement.onCommentModeChange().subscribe((mode) => {
   //mode will be `true` if the user has turned on the comment tool
   // [VELT]: We have used this method to change the CSS of the custom comment tool button based.
                setAddCommentMode(mode);

    // [VELT]: If the comment tool is active, pause the player and set the "location". Here Location refers to the current position of the lottie animation.
    if (mode) {
        pausePlayer();
        setLocation(client);
    }
...
## 8) Set Velt location to ensure comments are added to the right frame on the animation.

```jsx
const setLocation = (client) => {
    // get current frame
    console.log('setLocation');
    const currentFrame = playerRef?.current?.state?.instance?.currentFrame;
    if (currentFrame >= 0) {
        const location = { currentMediaPosition: Math.round(currentFrame) };
        if (client) {
  ...
  if (event) {
      ...
      if (location) {
          ...
          if (currentMediaPosition) {
              ...
              setSeeker(currentMediaPosition);

              // [Velt]: Step 10b: Set the Velt Location to the clicked comment location.
