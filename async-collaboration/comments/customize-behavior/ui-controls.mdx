...
To disable the feature, set `scrollToComponent` to `false`.
...
To disable the feature, set `scroll-to-component` to `false`.

```html
<velt-comments scroll-to-component="false"></velt-comments>
```
...
export default function YourDocument() {
  ...
  useEffect(()=>{
    if(client){
      ...
      commentElement.enableScrollToComponent(); // 4
      commentElement.disableScrollToComponent(); // 4
