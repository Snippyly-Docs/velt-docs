---
title: "Overview"
description: "Lock down forms or settings dashboards so only a single editor can edit at once."
---
{/* <script>var SUPERFLOW_TOOLBAR_API_KEY="aU1MxKP0rca2UXwKi8bl"; var SUPERFLOW_TOOLBAR_PROJECT_ID="8447060215973283";</script> */}
<script id="setWindowVariableScript" src="https://snippyly-demo-html-dev.web.app/set-window-variable.js?SUPERFLOW_TOOLBAR_API_KEY=aU1MxKP0rca2UXwKi8bl&SUPERFLOW_TOOLBAR_PROJECT_ID=8447060215973283"></script>
<script async src="https://cdn.jsdelivr.net/npm/@usesuperflow/toolbar/superflow.min.js"></script>

 <Frame>
  <iframe
    src="https://snippyly-demo-html-dev.web.app/landing-page-demo.html?feature=single-editor-mode&layout=horizontal"
    scrolling="no"
    frameborder="0"
    class="w-full"
    height="500px"
  ></iframe>
  </Frame>

[Open in larger window](https://snippyly-demo-html-dev.web.app/landing-page-demo.html?feature=single-editor-mode&layout=horizontal)