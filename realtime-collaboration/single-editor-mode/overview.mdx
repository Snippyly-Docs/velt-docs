...
For users in Read-Only mode, the elements listed above will be disabled, meaning `click`, `mouseup` and `mousedown` events will be prevented,
...
If there are any other elements beyond the elements we listed above, such as div or span elements, then you can add `data-velt-sync-access="true"` to that element.
