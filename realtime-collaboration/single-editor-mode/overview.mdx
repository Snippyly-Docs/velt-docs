...
### Custom Mode to Disable Input Elements in Single Editor Mode

Optionally, you can pass a `customMode` configuration object to enable `Custom Mode` on Single Editor Mode. 

If custom mode is enabled then we will not disable input elements from our end. You will have to manually disable input elements when the current user is not the editor.
...
The `requestEditorAccess().subscribe()` method is used to attempt to gain editor access from the current editor. 
...
The `isEditorAccessRequested().subscribe()` method is used to detect on the editor's client whether a non-editor has requested editor access or not. 
...
<Step title="To accept an editor access request">
...
<Step title="To reject an editor access request">
...
The `enableEditorAccessTransferOnTimeOut()` method is used to disable automatic transferring of editor access after timeout.
...
<Step title="Enable single editor UI">
...
<Step title="Disable single editor UI">
