...
#### (Optional) Make all tabs a separate editor

`Default: true`

You can pass a configuration object to make it so that each tab is treated as a separate editor. This way, if multiple tabs are open to the same page at the same time on the same browser, they will each have to request access to edit the page.
...
`isUserEditor()` returns a subscription that emits the following object:
...
export default function YourDocument() {
  ...
  let {editor, editorOnCurrentTab} = useUserEditorState()
  console.log("Is User Editor?: ", isEditor)

  return (
    <div>
        Is User Editor?: {editor}
    </div>
  )
}
```
...
It returns a subscription that emits the following object:

```jsx
{ requestStatus: string, requestedBy: User }
```

```jsx
liveStateSyncElement.isEditorAccessRequested().subscribe((data) => {
  // data.requestStatus == true -> Current user is Editor & access requested by Viewer
  // data.requestStatus == false -> Current user is not Editor
