...
`getEditor()` returns a subscription that emits the User object of the current editor. 

```jsx
liveStateSyncElement.getEditor().subscribe((user) => {
...
```jsx
import { useGetEditor} from '@veltdev/react';

export default function YourDocument() {

  let user = useGetEditor()
...
<Info>
If the user is not the editor or the request has been canceled, the hook return value will be null.
</Info>
